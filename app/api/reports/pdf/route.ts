...
export async function GET(request: Request) {
  ...
  const pdfBuffer = generatePdfReport(transactions, 'Laporan Keuangan Kas (Simple Finance)');

  createAuditLog({
    userId: session.id,
    action: 'EXPORT_PDF',
    module: 'REPORT',
    description: `Mengekspor ${transactions.length} data transaksi ke dokumen PDF`,
  });
